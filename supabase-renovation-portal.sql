...
create table if not exists user_profiles (
  ...
  generation_limit integer default 10, -- free tier limit
  created_at timestamp with time zone default now(),
  updated_at timestamp with time zone default now()
);
...
-- Create storage bucket for renovation images (run this separately in Supabase Dashboard > Storage)
-- Bucket name: 'renovation-images'
-- Public: false
-- File size limit: 10MB
-- Allowed MIME types: image/jpeg, image/png, image/webp

-- Storage policies (add after creating the bucket)
-- These allow authenticated users to upload and access their own images
...
-- Success message
select 'Renovation Portal database schema created successfully!' as message;
